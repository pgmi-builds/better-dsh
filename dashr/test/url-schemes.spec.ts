...
import { applySelector, parseUrl, UrlSchemesError } from '../src/url-schemes/selector.ts'
import { UrlResolver } from '../src/url-schemes/resolver.ts'
import type { ResolverEnv, SchemeHandler } from '../src/url-schemes/resolver.ts'
...
import { createDvcHandler, dispatchDvcWrite } from '../src/url-schemes/handlers/dvc.ts'
...
describe('parseUrl', () => {
  ...
  it('throws a structured error when there is no scheme', () => {
    expect(() => parseUrl('foo/bar')).toThrowError(UrlSchemesError)
  ...
  it('exposes the URL_NO_SCHEME code', () => {
    try {
      ...
      expect((err as UrlSchemesError).code).toBe('URL_NO_SCHEME')
    }
  })

  it('throws a structured error on a malformed line selector', () => {
    expect(() => parseUrl('skill://foo:abc')).toThrowError(UrlSchemesError)
  })
})
...
describe('UrlResolver', () => {
  ...
  it('throws a structured error for an unregistered scheme', async () => {
    const resolver = new UrlResolver()
    await expect(resolver.resolve(env, 'https://example.com')).rejects.toThrowError(UrlSchemesError)
  ...
  it('exposes the URL_UNREGISTERED_SCHEME code', async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(UrlSchemesError)
      expect((err as UrlSchemesError).code).toBe('URL_UNREGISTERED_SCHEME')
    }
  })
  ...
  it('treats history:// as an ordinary unregistered scheme (no alias hint)', async () => {
    ...
    for (const url of ['history://abc', 'history://']) {
      try {
        ...
        expect(err).toBeInstanceOf(UrlSchemesError)
        const coded = err as UrlSchemesError
