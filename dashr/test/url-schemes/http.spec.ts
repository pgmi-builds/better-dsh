...
import { createHttpHandler, HTTP_SCHEMES } from '../../src/url-schemes/handlers/http.ts'
import type { HttpResolverEnv } from '../../src/url-schemes/handlers/http.ts'
...
describe('http through UrlResolver: selector exemption', () => {
  ...
  it('parses port and query URLs with no selector at all', async () => {
    const { parseUrl } = await import('../../src/url-schemes/selector.ts')
