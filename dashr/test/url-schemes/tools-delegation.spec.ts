...
import { createGlobTool } from '../../src/url-schemes/tools/glob.ts'
import { createGrepTool } from '../../src/url-schemes/tools/grep.ts'
...
describe('write tool delegation', () => {
  ...
  it('rejects ctx:// writes with URL_READ_ONLY and the curated-snapshot reason', async () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(UrlSchemesError)
      const err = error as UrlSchemesError
