/**
 * `dsh-url-schemes` plugin wiring spec: proves the `apply()` contract that the
...
import { createDvcHandler } from '../../src/url-schemes/handlers/dvc.ts'
import { UrlResolver } from '../../src/url-schemes/resolver.ts'

describe('dsh-url-schemes wiring smoke', () => {
  it('exports the cordis plugin shape with the agents service injected', () => {
    expect(plugin.name).toBe('dsh-url-schemes')
  ...
  it('dvc + ctx handlers resolve through a fresh resolver', async () => {
    ...
    resolver.register('ctx', createCtxHandler({ sessionPersistence: undefined }))
    ...
    // ctx:// session recall requires the persistence service (change
    // 2026-09-11-url-schemes-recallable-context): without it the structured
    // CTX_NO_PERSISTENCE error surfaces instead of a silent identity read.
    await expect(
      resolver.resolve(
        { agent: { id: 'a1', status: 'idle', options: {}, session: { header: { cwd: '/tmp' } } } },
        'ctx://session:raw',
      ),
    ).rejects.toMatchObject({ code: 'CTX_NO_PERSISTENCE' })
  })
  it('the edit post-execute hook attaches the lsp diagnostics summary for a landed edit', async () => {
    ...
    const dvc = await import('../../src/url-schemes/handlers/dvc.ts')
