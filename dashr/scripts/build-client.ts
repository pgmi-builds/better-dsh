...
const ENTRY = 'src/fallbacks/client/index.ts'
...
await build({
  ...
  dts: false,
  clean: false,
  // tsdown 0.15.x config: the `deps` (neverBundle/alwaysBundle) API is 0.22+
  // only — on 0.15 the block is silently ignored and production deps fall
  // back to auto-externalization (which is what leaked
  // `require("use-sync-external-store/shim/with-selector")` into the bundle).
  // `external` pins the loader-table entries; `noExternal` forces every
  // non-table module (incl. use-sync-external-store, a runtime dependency) to
  // inline — the browser loader cannot answer a require() it does not know.
