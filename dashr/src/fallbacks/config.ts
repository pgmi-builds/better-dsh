...
/**
 * Spec §4 defaults — `Config({})` must equal this. The feature is ENABLED by
 * default with a single-entry all-day chain: when the PRIMARY model fails
 * (AUTH/QUOTA/RATE_LIMIT), requests fall back to `deepseek-official/deepseek-v4-flash`
 * out of the box. The chain is overridable in the settings page; clearing
 * `rootChain` disables the fallback (no candidate survives).
 */
