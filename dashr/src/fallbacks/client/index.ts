 ...
 *   namespace directory) and the provider/model catalog stay on the
 *   `remote` namespaces (`remote.settings` / `remote.llm` / `remote.session`,
 *   see `fallbacks-store.ts`).
 ...
 */
export const inject = ['slots', 'locale', 'connection', 'remote', 'uiConversation', 'remote.settings', 'remote.llm', 'remote.session']
...
export function apply(ctx: ClientContext): void {
  ...
  // catalog still ride the `remote` namespaces (guide §9).
  const controller = new FallbacksSettingsController(ctx.remote as never, connection.rpc)
