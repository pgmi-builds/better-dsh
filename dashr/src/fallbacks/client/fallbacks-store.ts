...
export class FallbacksSettingsController {
   ...
   */
  constructor(
    private readonly remote: Pick<IApiClient, 'settings' | 'llm' | 'sessions'>,
  ...
  async loadCatalog(): Promise<void> {
    ...
    try {
      const [providersResponse, modelsResponse] = await Promise.all([
        ...
        this.remote.session.modelCatalog(),
      ])
      if (generation !== this.catalogGeneration) return
      if (!providersResponse.ok) throw providersResponse.error
      if (!modelsResponse.ok) throw modelsResponse.error
      const providers = providersResponse.value
      const groups = modelsResponse.value.groups
      const failures = modelsResponse.value.failures
      this.store.update((state) => {
  ...
  async loadSwitches(): Promise<void> {
    ...
    try {
      // The `sessions.history` wire face was removed in 0.1.2-alpha (transcript
      // events moved off the Remote boundary). The recent-switch summary
      // degrades to empty until a full client-half migration restores it; the
      // form itself is unaffected (config + catalog ride the remote + gateway).
      if (generation !== this.switchesGeneration) return
      const switches: never[] = []
      this.store.update((state) => {
        state.switchesStatus = 'ready'
        state.switchesError = null
