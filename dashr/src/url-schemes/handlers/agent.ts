...
function requireAddress(
  ...
  if (target === undefined) throw new UrlSchemesError('AGENT_UNKNOWN_ID', message)
  return target
}
...
function unknownId(target: AddressResolution, message: string): UrlSchemesError {
  ...
  return new UrlSchemesError('AGENT_UNKNOWN_ID', `${message}${note}`)
}
