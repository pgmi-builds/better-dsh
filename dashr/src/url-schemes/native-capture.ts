 ...
 *
 * - The URL-aware `read`/`write`/`grep`/`glob` tools are delegation shells:
 *   non-URL inputs must run through the definition registered under the same
 *   SEMANTIC NAME at capture time (the native tool, or another feature's
 *   wrapper — capture anchors on the name, never on an implementation). The
 *   capture must happen before the chassis/wrappers register on the agent's
 *   own scope layer — `ctx.tools.get(name, agent)` resolves scoped-shadowed
 *   names, so a capture taken after registration would resolve back to the
 *   wrapper itself and delegate into infinite recursion. The wiring step
 *   therefore calls {@link captureNativeTools} at `agent/session-start`, then
 *   registers the wrappers with the captured set.
...
/** Tool names the chassis/wrappers shadow and delegate back to. */
const NATIVE_TOOL_NAMES = ['read', 'write', 'grep', 'glob'] as const
