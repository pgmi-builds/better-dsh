 ...
 * {@link UrlSchemesError} (`LSP_*`); through the dispatcher these surface as
...
function resolveServerConfig(args: Record<string, unknown>, filePath: string): { name: string; config: ServerConfig } {
  ...
  if (server !== undefined) {
    if (typeof server !== 'string' || server === '') {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "server" must be a non-empty server name')
    }
    const config = serverByName(server)
    if (config === undefined) {
      throw new UrlSchemesError('LSP_NO_SERVER', `lsp device: no defaults.json entry named "${server}"`)
    }
  ...
  if (command !== undefined) {
    if (typeof command !== 'string' || command === '') {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "command" override must be a non-empty string')
    }
    const commandArgs = args.args
    if (commandArgs !== undefined && (!Array.isArray(commandArgs) || commandArgs.some(a => typeof a !== 'string'))) {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "args" override must be an array of strings')
    }
    return {
      name: command,
      config: {
...
const lspDevice: DvcDevice = {
  async execute(args: unknown): Promise<unknown> {
    ...
    if (typeof file !== 'string' || file === '') {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: this action requires a non-empty "file" string')
    }
    const content = record.content
    if (content !== undefined && typeof content !== 'string') {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "content" override must be a string')
    }
    const saved = record.saved
    if (saved !== undefined && typeof saved !== 'boolean') {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "saved" flag must be a boolean')
    }
    if (saved === true && content === undefined) {
      throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "saved" requires a "content" override (the save signal rides a content sync)')
    }
    ...
    if (content === undefined && !existsSync(filePath)) {
      throw new UrlSchemesError('LSP_BAD_ARGS', `lsp device: file not found: ${filePath}`)
    }
    ...
    switch (action) {
      ...
      case 'format': {
        if (content === undefined) {
          throw new UrlSchemesError('LSP_BAD_ARGS', 'lsp device: "format" requires a "content" string to format')
        }
      ...
      default:
        ...
        throw new UrlSchemesError('LSP_BAD_ARGS', `lsp device: unhandled action "${String(action)}"`)
    }
  },
  summary:
