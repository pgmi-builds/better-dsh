 ...
 * throws a structured {@link UrlSchemesError} with the underlying cause
 ...
 * dependency must still load the whole dsh-url-schemes plugin — only the browser
...
function withCause(error: UrlSchemesError, cause: unknown): UrlSchemesError {
  error.cause = cause
  return error
}
...
async function gotoUrl(target: Page, url: string): Promise<void> {
  try {
    ...
    throw withCause(
      new UrlSchemesError('BROWSER_GOTO_FAILED', `browser device: goto(${url}) failed: ${messageOf(error)}`),
      error,
    )
  }
}
...
async function runAction(args: Record<string, unknown>): Promise<unknown> {
  ...
  if (hasCode) {
    ...
    try {
      ...
      throw withCause(
        new UrlSchemesError('BROWSER_EVAL_FAILED', `browser device: evaluate failed: ${messageOf(error)}`),
        error,
      )
    }
  }
  ...
  try {
    ...
    if (error instanceof UrlSchemesError) throw error // goto failures are already structured
    throw withCause(
      new UrlSchemesError('BROWSER_STEP_FAILED', `browser device: run step failed: ${messageOf(error)}`),
      error,
    )
  }
  return { ok: true, url: target.url(), title: await target.title() }
