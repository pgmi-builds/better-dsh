 ...
 *   - `:raw:N-M`                  → composite form: the `:raw` prefix is redundant in
 *                                    a line-window context but MUST parse — `:raw:N-M`
 *                                    is exactly `:N-M` (§15 grammar; line windows
 *                                    always index the canonical full content)
...
import { SCHEME_DISPLAYS } from './catalog.ts'
...
export class UrlSchemesError extends Error {
...
export function parseUrl(raw: string): ParsedUrl {
  ...
  if (m === null) {
    throw new UrlSchemesError(
      'URL_NO_SCHEME',
      `URL "${raw}" has no scheme — expected "scheme://" (${SCHEME_DISPLAYS.join(', ')})`,
    )
  }
  const scheme = m[1]!
  const rest = m[2]!
...
function parseColonSelector(selPart: string): Selector {
  ...
  // Composite form: a `raw:` prefix followed by a line-window spec parses
  // identically to the bare window (`:raw:1-10` ≡ `:1-10`); `applyFace` applies
  // `lines` to the canonical parameter either way, so semantics are unchanged.
  if (selPart.startsWith('raw:')) return { kind: 'lines', ranges: parseRanges(selPart.slice('raw:'.length)) }
