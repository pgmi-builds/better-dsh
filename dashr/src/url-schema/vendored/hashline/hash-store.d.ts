...
export interface HashStore {
    ...
    getServed(path: string): (string | null)[];
    /** The reported-drift hash set for a session+path (lenient parse, never deletes). */
    getServedReported(path: string): Set<string>;
    /** Persist the hashes JSON column for a session+path. */
    upsertServed(path: string, hashesJson: string): void;
    /** Persist the reported-drift JSON column for a session+path (inserting a fresh empty hashes row). */
    upsertServedReported(path: string, reportedJson: string): void;
    clearServedReported(path: string): void;
    deleteServed(path: string): void;
