...
### 9.3 4999 CDP 双形态（lead）—— TODO（tasks 2.2）

- [ ] `Emulation.setEmulatedMedia(features:[{name:'display-mode',value:'standalone'}])`：standalone+iOS+390 → meta stock、`#ios-zoom-font-floor` 在场、composer computed font-size ≥16px；browser（无 emulated media）→ meta guarded、无 font-floor style；Android/桌面/宽屏双形态均双零；off 双零。
- [ ] client 特性探针（本轮回归教训）：narrow 下 `[data-sidebar-collapsed]` computed 首列 0px、`style[data-plugin]` 认领在位。
- [ ] 重启前后 boot graph rev 变化确认（tsdown + tsx build-client 双步已做，lib/client 在场 md5 `a88850ec…`）。
