...
- **Ruling P20**（user 2026-09-24，name-first 策略）：裸名**先按名匹配**——ssh config 字面 host → docker → incus（与 roster 同源的本地廉价扫描，撞名 ssh 优先，查无 → 默认 ssh 交原生错误）；选择器（ssh:/docker:/incus:）降级为显式消歧备用，描述不再当主路径宣传。修正 P18 的过度矫正（裸名 ≠ 恒 ssh，容器名裸传即可达）。若错：代价=裸名多一次本地扫描（亚秒），回退为恒 ssh 一行。
