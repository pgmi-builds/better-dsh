...
- [ ] 1.2 restrict 接线：`agent/session-start` 末尾 `agentCtx.tools.restrict({deny: WIRE_MASKED_NAMES})`（默认名单=design D1：`skill`/`send_message`/`report`/`list_agents`/`subagent`/`subagent_fork`/`interrupt_agent`/`workflow`/`ralph` 全量，config 可覆盖）；own-layer wrapper 注册先行（时序断言入测试）
