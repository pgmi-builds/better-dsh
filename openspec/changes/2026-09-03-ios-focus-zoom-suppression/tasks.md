...
- [ ] 2.3 **4999 验证**：重启 4999 后鉴权拉 shell 页 —— head 内联 boot script 含 zoomGuard 段且早于 application bundle 引用；CDP 视口模拟矩阵（iOS UA+390px → meta 含两 token；1280px → stock；Android UA+390px → stock；zoomGuard off → stock）；桌面回归。
