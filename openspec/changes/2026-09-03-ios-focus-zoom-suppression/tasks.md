...
- [x] 2.2 **boot script 接线**：内联脚本增 zoomGuard 段 —— 记录 stock content → 门控判定（iOS && matchMedia(max-width: breakpoint-0.02px) && zoomGuard==='meta'）→ 改写/还原 → `resize` 复评；维持无依赖纯同步风格；注入行形状单测（payload 序列化 + 脚本片段形状）。
  ...
  - 返修（2026-09-03，lead 4999 CDP blocker：初始窄视口载入 guard 缺席）：boot script 早于引擎应用 viewport meta（布局视口首评为 pre-meta 默认宽 980）→ 首评 false，且 meta 应用期的收窄在载入期不派发 `resize` → resize 监听永不触发。修复 = ①载入重评梯子（`setTimeout` ~10ms poll `re()`，至 `applied===true` / `readyState==='complete'` / ~2s（200 tick）上限任一即停；`apply(true)` 主动 cancel 待发 tick，停后零残留 timer；稳态 tick = 幂等 no-op）+ ②mq `change` 监听兜底（`addEventListener` 缺席回落 `addListener`，与 window resize 双通道）。既有语义不变（iOS 早退零监听零 timer、off 不发射、单 meta、还原字节级、风暴幂等）；spec 增 6 场景（梯子落位无 resize / 梯子 provisional 路径 / readyState 停止+通道存活 / 非 iOS 零 setTimeout / mq change 双向 / legacy addListener）；全量 466/466 + tsc 0；tsdown 后 `.scratch/zg-built-check.ts`（注入 timer 队列驱动梯子）对 monorepo 产物全绿。
- [x] 2.3 **4999 验证**：重启 4999 后鉴权拉 shell 页 —— head 内联 boot script 含 zoomGuard 段且早于 application bundle 引用；CDP 视口模拟矩阵（iOS UA+390px → meta 含两 token；1280px → stock；Android UA+390px → stock；zoomGuard off → stock）；桌面回归。
  - 证据（lead，2026-09-03 晚）：冒烟四断言过（段在场/graph+URL/时序行为成立/client.js 单包 200 前缀字节一致+84B）；首轮矩阵 7/9 暴露 blocker（初始窄载入不 guard：引擎应用 viewport 期间不派发 resize，deferred focus 可先于补评）→ 返修 `59870fc`（早期重评梯子 + MQ change 通道，466/466）→ 终验 **9/9**；off 逃生门经 home 层覆盖 live PASS（整段不发射）；默认态恢复复验 + 桌面 user 复验 PASS。报告 §八；脚本 `.scratch/zg-verify/`。
