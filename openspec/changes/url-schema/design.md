...
| `agent://`, `agent://<id>`, `<id>/transcript`, `<id>/<child>` | roster table / output artifact / transcript / nested output | `ctx.sessions`, `ctx.subagents`, `ctx.agents` | roster has five columns: id, status, kind, parent, last activity; `status` from the live agent registry (`-` when not live), `kind` = `header.origin` (default `main`), `parent` = `header.parentSession` (`-` at top level), last activity = last session event time (ISO 8601, `-` when empty); output = last non-empty assistant message (`SubagentResult.output` semantics). **Liveness constraint (field-verified)**: `<id>/<child>` requires the child to be LIVE in the session store — a settled one-shot subagent reports `child agent … is not live in the session store` (short id → `unknown child agent`), so a finished one-shot child's final output is not retrievable via URL. Falling back to archived `ctx.subagents` output is a deferred enhancement, not a defect. One-shot subagent sessions also do not appear in the roster (they are absent from `ctx.sessions`). |
...
**Empirical boundary (v0.1.8d field probe)**: in `both` presentation mode the mask removes ADVERTISING, not the execution path — a model-direct native call to the masked `skill` executes in full (probe: `skill({"name":"grilling"})` returned the complete `<skill_content>` block), stronger than ADR-0002's "reachable via nested sub-dispatch": no parent token needed. Causal chain: the mask registers no visibility filter, `view(scope).visible` always contains the name, and `resolveExecution` only collapses model-direct calls in `code` mode. The hard gate is L2 (REPL bindings are an explicit allowlist; masked names fail with `unknown binding`). This is a CONSCIOUS security boundary, not an oversight: true model-direct rejection would need a visibility-layer `restrict()` (rejected by ADR-0002 for ordering hazards) or a `code`-mode collapse (a deployment-wide change). Recorded so nobody mistakes the mask for an enforcement point.
...
**Selector exemption — RESOLVED (post-verifier fix, landed v0.1.8d)**: `parseUrl` skips selector extraction for `http`/`https` (`SELECTOR_EXEMPT` in `selector.ts`) — `:` and `?` in those URLs belong to the URL itself (port, server query), so the whole remainder is the path and no selector is parsed. Port-carrying URLs resolve end-to-end and query strings are never re-applied as content filters. Regression tests go through the real `UrlResolver` (the original handler-direct tests bypassed the shared parser and missed this). Verified in the field: `https://example.com:443/` fetches; `https://example.com/?x=1` returns the full body with no double filtering.
...
- [Selector/URL syntax collision] RESOLVED by the D9 selector exemption (`SELECTOR_EXEMPT`): `http`/`https` URLs carry their `:`/`?` verbatim; no other scheme is affected.
...
- **http selector exemption** — RESOLVED (landed v0.1.8d): `parseUrl` skips selector extraction for `http`/`https` (`SELECTOR_EXEMPT` in `selector.ts`); port and query URLs resolve end-to-end through the real resolver path. See D9.
