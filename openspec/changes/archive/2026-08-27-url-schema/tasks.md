...
- [x] 9.14 Field-test doc sync (v0.1.8d 实测报告): design.md D9 + Open Questions + Risks updated — `:port`/`?query` exemption is implemented and field-verified, open question closed; D3 table annotates the `<id>/<child>` liveness constraint (settled one-shot subagents not retrievable via URL; roster omits them; archived-output fallback deferred); D5 + ADR-0002 record the field-probed mask boundary (in `both` mode the mask is an advertising cut — model-direct native calls still execute; hard gate is the REPL binding allowlist). Doc-only change; no code touched.
