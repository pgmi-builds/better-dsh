...
- [x] 1.5 **4999 + prod 双实测**：fence 腿（`DSH_TRUSTED_HOSTS` + 非 loopback Host 的 `/api` 403→200）与 isLoopback 腿（`trustedPageAuthorities` 含 LAN IP / 域名时，非 loopback 拼法访问 Settings/Models 恢复、持久化 host 生效）分别验证；空配置双惰性复测；含 prod alpha.3 手改 patch 现状核验（retired 路径）。  # fence/isLoopback 双腿实证：报告 T5（DSH_TRUSTED_HOSTS=probe.example → Host 401/403）+ Round-2 LAN socat 与 CDP 真浏览器 Models；Round-4 user 真实 URL 复验 "UI ok"；prod 随 0.2.3a/0.2.3-b 部署在位（DSH_TRUSTED_HOSTS 生效）
...
- [x] 2.2 **CSS 覆盖**：媒体查询 + `[data-sidebar-collapsed]`（及平板窄档策略）→ 侧栏轨 0；验证矩阵：手机档（<920）/ 平板窄档（920–1023，details 开关两态）/ 桌面档（≥1024 不受影响）。  # narrow 探针（后轮次复验）：`[data-sidebar-collapsed]` computed 首列 0px + `style[data-plugin]` 认领在位；user 真机 "UI ok"（报告 Round-4）
- [x] 2.3 **手势三条件**：起点（边缘带）+ 距离 + **速率**判定纯函数化 + vitest 单测（慢速拖选不触发、快速轻扫触发、可交互元素起点忽略）。
- [x] 2.4 **4999 实测**：devtools 移动模拟 + 真机（视口 + 触摸）双验；`toggleSidebar` 窄视口语义（narrowExpanded 翻转）行为正确。  # 4999 devtools 移动模拟（v0.2.1f 报告）+ user 真机（手机/MacBook，test.pc.randomhash.app）Round-4 验收 "UI ok"
