...
- `@deepseek-ai/dsh` = **0.1.6-alpha.2**（2026-09-21 user 裁决对齐；旧值 0.1.3-alpha.2——侧栏 Plugins/plugin-manager UI 仅存在于 0.1.6 线），自带 vendored `node_modules`（内含 `dsh-plugin-manager` 0.1.6-alpha.2 host 服务 + `dsh-client-ui-plugin-manager` 页面）。`npm install -g --prefix ~/.local` 的用户级全局安装。
